...
class viewer(Callback):
    ...
    def on_epoch_begin(self, epoch, logs= None):
        ...
        #-----BUILD THE CANVAS-----#
        canvas = Image.new(mode = 'RGB', size = (1160, 340), color = (221,221,212))
        ...
        draw.text((345,10), "Epoch number: " + str(epoch), fill=(255,0,0,128))
        draw.text((50,30), "Train sample", fill=(255,0,0,128))
        draw.text((100+photo_width,30), "Target", fill=(255,0,0,128))
        draw.text((150+2*photo_width,30), "STN prediction: " + str(epoch), fill=(255,0,0,128))
...
#W = np.zeros((100, 9), dtype='float32')
weights = [W, b.flatten()]
...
transf1.compile(loss='mse', optimizer='adam', metrics=['mse'])
locnet.summary()
transf1.summary()
...
transf1.fit_generator(generator = fgenerator(X,y, batch_size), steps_per_epoch = len(X)/batch_size, \
        epochs = 50, shuffle = True, validation_data = vgenerator(X_val, y_val, batch_size),\
